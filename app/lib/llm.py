...
# 8/17/2023: Now done in __init__.py
...
def construct_ai_prompt(
    chat: list[dict[str, str]], patent: dict | str
) -> PromptValue:
    ...
    sys_msg_prompt_template = SystemMessagePromptTemplate.from_template(
        sys_msg_template
    )
    ...
    for i, msg in enumerate(chat):
        ...
        messages.append(
            AIMessage(content=msg["msg"])
            if msg["source"] == "ai"
            else HumanMessage(content=msg["msg"])
        )
    ...
    chat_prompt = chat_prompt_template.format_prompt(
        ...
        unique_words=json.dumps(unique_word_lists),
    ).to_messages()
    return chat_prompt
...
def get_unique_words_per_indep_claim(claims: str) -> dict[str, list[str]]:
    ...
    indep_claim_indices = [
        ...
        if re.search(dep_claim_pattern, c) is None
    ]
    ...
    for i, c in enumerate(claims_list):
        if i in indep_claim_indices:
            ...
            assert (
                claim_num is not None
            ), "Could not find independent claim number."
            ...
            processed_indep_claim_indices = list(
                indep_claims_and_their_dependents.keys()
            )
            processed_indep_claim_indices.sort()
            latest_indep_claim_index = processed_indep_claim_indices[-1]
            indep_claims_and_their_dependents[latest_indep_claim_index] = (
                indep_claims_and_their_dependents[latest_indep_claim_index] + c
            )

    # Get set of unique words for each independent claim and its dependent claims.
    word_sets = {
        ic_num: get_word_set(claims_text)
        for ic_num, claims_text in indep_claims_and_their_dependents.items()
    }
